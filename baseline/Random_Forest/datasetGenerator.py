...
class WindowGenerator:
    """
    随机森林版本：生成滑动窗口并展平 (Flatten) 为 2D 矩阵
    """
    ...
    def make_dataset(self, df_processed):
        print(f"正在生成滑动窗口数据 (窗口大小={self.seq_len}) 用于随机森林...")
        ...
        print(f"使用特征列 ({len(feature_cols)}): {feature_cols}")
        ...
        for name, group in tqdm(grouped, desc="处理卫星序列"):
            ...
            for i in range(num_records - self.seq_len + 1):
                ...
                # --- 关键步骤：展平 (Flatten) ---
                # 将 (Sequence_Length, Num_Features) -> (Sequence_Length * Num_Features)
                # 例如 (10, 7) -> (70,)
        ...
        print(f"生成完成。X形状: {X_final.shape}, y形状: {y_final.shape}")
