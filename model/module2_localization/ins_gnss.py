# module2_localization/ins_gnss.py — INS/GNSS loosely-coupled integration (stub)
# ponytail: INS needs IMU data not in current dataset; stub returns LS fallback.
import numpy as np
from .base import LocalizationBase
from .factory import LocalizationFactory


@LocalizationFactory.register("ins_gnss")
class INSGNSS(LocalizationBase):
    """INS/GNSS loosely-coupled integration. Stub — falls back to LS."""
    ...
    def solve(self, observations, sv_positions, sv_systems=None, additional_info=None):
        ...
        details = {"converged": True, "iterations": 10, "residuals": residuals,
                   "note": "INS/GNSS stub — LS fallback"}
