...
# P2:   Multi-optimizer support — L-BFGS-B, trust-ncg, Newton-CG
# ================================================================
import numpy as np
...
def _smooth_max(x, lo, k=5.0):
    ...
    t = k * (x - lo)
    # for large t: log(1+exp(t)) ≈ t, for small t: use softplus
...
class MoGObservationModel:
    ...
    def log_likelihood_smooth(self, residuals):
        '''Smooth MoG log-likelihood — no hard clips.'''
    ...
    def neg_log_likelihood_and_grad(self, x, sv_positions, pr_measured):
        ...
        # Stable sigmoid: at z>>3 → 0.3, at z=0 → 1.0
        x = 5.0 * (z_score - 3.0)
