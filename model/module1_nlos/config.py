...
class Config:
    ...
    RESULT_DIR = os.path.join(PROJECT_ROOT, "model", "module1_nlos", "result")
    ...
    EARLY_STOPPING_PATIENCE = 200  # default; overridden to 60 for MoG training
    ...
    MU_NLOS_MIN = 0.0
    MU_NLOS_MAX = 3.0  # [v5] physical ceiling (km)
    ...
    # ========== Logging Config ==========
    LOG_INTERVAL = 5
    USE_TENSORBOARD = True  # sandbox blocks TB write
    ...
    # ========== Per-Dataset Config Overrides (P0: Frankfurt p_los/NLL fix) ==========
    # Applied in GAT_V2025.py main() after AUTO_POS_WEIGHT computation.
    # Keys override Config attributes for specific datasets.
    DATASET_OVERRIDES = {
        'frankfurt1_maintower': {
            'LAMBDA_ENTROPY': 0.005,        # lower entropy reg → allow more extreme p_los
            'SIGMA_NLOS_CLAMP_LOG_MAX': 3.5,  # increase sigma_nlos ceiling (exp 3.5 ≈ 33 km)
        ...
        'frankfurt2_westendtower': {
            ...
            'SIGMA_NLOS_CLAMP_LOG_MAX': 3.5,
            'LAMBDA_SIGMA_REG': 0.02,
            'SIGMA_GAP_TARGET': 1.0,
        },
    }
