...
class NLOSGAT(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        # Fix 6B: configurable log-space clamp (P0: per-dataset overrides)
        los_min = getattr(self, 'sigma_los_clamp_log_min', -3.0)
        los_max = getattr(self, 'sigma_los_clamp_log_max', 2.0)
        nlos_min = getattr(self, 'sigma_nlos_clamp_log_min', -3.0)
        nlos_max = getattr(self, 'sigma_nlos_clamp_log_max', 2.5)
        log_sigma_los = torch.clamp(log_sigma_los_raw, min=los_min, max=los_max)
        log_sigma_nlos = torch.clamp(log_sigma_nlos_raw, min=nlos_min, max=nlos_max)
...
class SupervisedMuRegressionLoss(nn.Module):
    """[v5] Direct supervision for mu_nlos using ground truth pseudorange errors.

    Only supervises on NLOS samples. Target = clamped positive pseudorange error.
    """
    def __init__(self):
        super().__init__()

    def forward(self, mu_nlos, pseudorange_error, nlos_labels):
        nlos_mask = (nlos_labels.squeeze() == 1)
        if not nlos_mask.any():
            return torch.tensor(0.0, device=mu_nlos.device)

        mu_target = torch.clamp(pseudorange_error.squeeze()[nlos_mask], min=0.0, max=3.0)
        mu_pred = mu_nlos.squeeze()[nlos_mask] if mu_nlos.dim() > 1 else mu_nlos[nlos_mask]
        # Huber loss (robust to outliers)
        loss = F.huber_loss(mu_pred, mu_target, delta=0.3)
        return loss

class MuDirectionLoss(nn.Module):
    """[v8] Pure pairwise ranking loss: enforce mu_nlos[NLOS] > mu_nlos[LOS].

    NO magnitude suppression - only relative ordering is constrained.
    This preserves mu magnitude while fixing direction (fixes v7 collapse).

    Uses:
      1. Mean ordering: mean(mu_NLOS) > mean(mu_LOS) + margin
      2. Pairwise sampling: random pairs for richer gradient signal
    """
    def __init__(self, ordering_margin=0.15):
        super().__init__()
        self.margin = ordering_margin

    def forward(self, mu_nlos, nlos_labels):
        los_mask = (nlos_labels.squeeze() == 0)
        nlos_mask = (nlos_labels.squeeze() == 1)

        if not (los_mask.any() and nlos_mask.any()):
            return torch.tensor(0.0, device=mu_nlos.device)

        mu_los = mu_nlos.squeeze()[los_mask]
        mu_nlos_vals = mu_nlos.squeeze()[nlos_mask]

        # Mean ordering: mean(mu_NLOS) > mean(mu_LOS) + margin
        mu_los_mean = mu_los.mean()
        mu_nlos_mean = mu_nlos_vals.mean()
        ranking_loss = torch.relu(self.margin - (mu_nlos_mean - mu_los_mean))

        # Pairwise sampling: richer gradient signal than mean comparison
        ...
        if n_pairs > 0:
            ...
            pair_loss = torch.relu(
                pair_margin - (mu_nlos_vals[idx_nlos] - mu_los[idx_los])
            ).mean()
            loss = 0.5 * ranking_loss + 0.5 * pair_loss
        else:
            loss = ranking_loss
...
def train_epoch(model: nn.Module, dataloader: DataLoader,
                ...
                mog_blend_epochs: int = 15,
                 ...
                 bce_only_loss: Optional[nn.Module] = None,
                 mu_reg_loss_fn: Optional[nn.Module] = None,
                 mu_direction_loss_fn: Optional[nn.Module] = None) -> Dict[str, float]:
    """Train one epoch"""
    model.train()
    total_loss = 0.0
    total_bce = 0.0
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if use_mog_loss_fn:
            if is_pure_bce:
                ...
                # [v7] Mu direction constraint in blend stage
                loss_mu_dir = torch.tensor(0.0, device=device)
                if mu_direction_loss_fn is not None:
                    loss_mu_dir = mu_direction_loss_fn(mu_nlos, nlos_labels)
                loss = loss + 0.5 * loss_mu_dir
                ...
                # [v5] Supervised mu_nlos regression
                loss_mu_reg = torch.tensor(0.0, device=device)
                if mu_reg_loss_fn is not None:
                    loss_mu_reg = mu_reg_loss_fn(mu_nlos, pseudorange_errors, nlos_labels)
                ...
                # [v7] Add supervised mu regression + Mu direction constraint
                loss_mu_dir_nll = torch.tensor(0.0, device=device)
                if mu_direction_loss_fn is not None:
                    loss_mu_dir_nll = mu_direction_loss_fn(mu_nlos, nlos_labels)
                loss = loss + 0.5 * loss_mu_reg + 1.0 * loss_mu_dir_nll
        else:
            try:
...
def main(resume_from: str = None, num_epochs: int = None, dataset_name: str = None, exp_name: str = None):
    ...
    # P0: Apply per-dataset config overrides (Frankfurt p_los/NLL fix)
    if dataset_name in getattr(config, 'DATASET_OVERRIDES', {}):
        overrides = config.DATASET_OVERRIDES[dataset_name]
        print(f'  Applying dataset overrides for {dataset_name}:')
    ...
    # P0: Configurable sigma clamp attributes (per-dataset overrides)
    model.sigma_los_clamp_log_min = config.SIGMA_LOS_CLAMP_LOG_MIN
    model.sigma_los_clamp_log_max = config.SIGMA_LOS_CLAMP_LOG_MAX
    model.sigma_nlos_clamp_log_min = config.SIGMA_NLOS_CLAMP_LOG_MIN
    model.sigma_nlos_clamp_log_max = config.SIGMA_NLOS_CLAMP_LOG_MAX
    ...
    mu_reg_loss_fn = SupervisedMuRegressionLoss().to(device)  # [v5]
    mu_direction_loss_fn = MuDirectionLoss(
        ordering_margin=config.MU_DIRECTION_MARGIN,
    ).to(device)  # [v8] pairwise ranking only, no LOS suppression
    ...
    for epoch in range(start_epoch, total_epochs):
        train_metrics = train_epoch(
            ...
            mu_reg_loss_fn=mu_reg_loss_fn,
            mu_direction_loss_fn=mu_direction_loss_fn,
        )
        ...
        if val_metric_for_best < best_val_loss:
            ...
            _safe_torch_save({
                ...
                'sigma_clamp_attrs': {
                    'los_min': getattr(model, 'sigma_los_clamp_log_min', -3.0),
                    'los_max': getattr(model, 'sigma_los_clamp_log_max', 2.0),
                    'nlos_min': getattr(model, 'sigma_nlos_clamp_log_min', -3.0),
                    'nlos_max': getattr(model, 'sigma_nlos_clamp_log_max', 2.5),
                },
