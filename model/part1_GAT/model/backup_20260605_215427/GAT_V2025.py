...
class MuDirectionLoss(nn.Module):
    """[v7] Enforce mu_nlos[NLOS] > mu_nlos[LOS] directional constraint.

    Problem: MoGNLL optimizes mu_nlos upward for LOS samples to push the
    NLOS Gaussian away from zero, improving LOS likelihood. This creates
    a direction inversion where mu_nlos[LOS] > mu_nlos[NLOS].

    Fix:
      1. LOS suppression: mu_nlos[LOS] should be near zero
      2. Ordering: mean(mu_NLOS) > mean(mu_LOS) + margin
    """
    def __init__(self, los_target=0.05, ordering_margin=0.10):
        super().__init__()
    ...
    def forward(self, mu_nlos, nlos_labels):
        ...
        # LOS suppression: mu_nlos should be near zero for LOS samples
        if los_mask.any():
            los_excess = torch.relu(mu_nlos.squeeze()[los_mask] - self.los_target)
            loss = loss + 2.0 * los_excess.mean()

        # Ordering constraint: mean(mu_NLOS) > mean(mu_LOS) + margin
        if los_mask.any() and nlos_mask.any():
            mu_los_mean = mu_nlos.squeeze()[los_mask].mean()
            mu_nlos_mean = mu_nlos.squeeze()[nlos_mask].mean()
            ordering_violation = torch.relu(mu_los_mean - mu_nlos_mean + self.margin)
